...
Sensors          & 3$\times$ Stereolabs ZED X stereo cameras (front-left, front, front-right), Velodyne VLP-16 3D LiDAR, Xsens MTi-680G GNSS/INS \\
...
\begin{itemize}[nosep]
    \item \textbf{Sensors} --- Stereo camera capture (2$\times$ ZED X), 3D LiDAR point clouds (Velodyne VLP-16), GNSS/INS readings (Xsens MTi-680G), motor encoder feedback
    \item \textbf{Perception} --- GPU-accelerated HSV lane detection on stereo camera feeds and LiDAR-based obstacle detection (Jetson Orin)
    \item \textbf{Localization} --- Sensor fusion of track odometry, GNSS position, and 9-axis inertial data for position and heading estimation
...
\midrule
NVIDIA Jetson Orin AGX           & 1 & \$TODO & \$TODO \\
Stereolabs ZED X stereo camera   & 3 & \$TODO & \$TODO \\
Velodyne VLP-16 3D LiDAR         & 1 & \$TODO & \$TODO \\
Xsens MTi-680G GNSS/INS          & 1 & \$TODO & \$TODO \\
...
A sensor mast is mounted at the center/front of the robot to elevate the three ZED X stereo cameras, the Velodyne VLP-16 3D LiDAR, and the Xsens MTi-680G GNSS antenna. The ZED X cameras are mounted as an angled trio---one facing forward and two angled outward to the front-left and front-right---to maximize lateral coverage of lane lines on both sides of the vehicle while preserving a dedicated forward view for long-range lane tracking. The Velodyne VLP-16 is mounted above the cameras to preserve an unobstructed 360$^\circ$ horizontal field of view for obstacle detection. Elevating the cameras provides a wider field of view and allows the image to be projected onto a 2D plane with minimal distortion, while elevating the GNSS antenna reduces multipath interference.
...
\begin{itemize}[nosep]
    \item \textbf{NVIDIA Jetson Orin AGX} --- Main onboard GPU computer running HSV lane detection on the ZED X stereo feeds and LiDAR-based obstacle detection
    \item \textbf{Teensy 4.1} --- Single microcontroller node acting as the CAN bridge between the Jetson and the REV SparkMAX motor controllers, handling motor commands and encoder feedback
    ...
    \item \textbf{3$\times$ Stereolabs ZED X} --- GMSL2 stereo cameras (front-left, front, front-right) for lane detection
    \item \textbf{Velodyne VLP-16} --- 16-channel 360$^\circ$ 3D LiDAR for obstacle detection, connected via Ethernet
    \item \textbf{Xsens MTi-680G} --- GNSS/INS unit providing GPS fixes and 9-axis inertial data (RTK disabled per IGVC rules)
    \item \textbf{Router} --- Central network hub connecting Jetson Orin, Velodyne VLP-16, and Teensy 4.1 via Ethernet, with Wi-Fi to remote laptop
    \item \textbf{E-stop system} --- Hardware relay controlled by physical + wireless + software interlocks
    \item \textbf{Safety lights} --- RGB indicator lights driven from the Jetson
\end{itemize}
...
\midrule
Jetson Orin AGX            & 19 (via buck)          & 4.0  & 2.0 \\
Teensy 4.1                 & 5 (via 5\,V/6\,A buck) & 0.1  & 0.05 \\
SparkMAX + motors          & 12 (via 12\,V/20\,A buck) & TODO & TODO \\
3$\times$ ZED X cameras    & 12                     & 2.3  & 1.8 \\
Velodyne VLP-16            & 12                     & 0.8  & 0.6 \\
Xsens MTi-680G             & 5 (USB)                & 0.2  & 0.15 \\
Router                     & 12                     & 0.8  & 0.5 \\
Safety lights              & 12                     & 0.5  & 0.3 \\
\midrule
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/perception_pipeline.pdf}
    \caption{Perception pipeline. Two parallel branches process data from the three ZED X stereo cameras (HSV lane detection) and the Velodyne VLP-16 LiDAR (3D obstacle detection), merging into a unified costmap consumed by the planner.}
    \label{fig:perception_pipeline}
\end{figure}

\subsection{Sensor Suite}

\textit{ROBOT NAME} uses a complementary sensor suite that splits perception responsibilities between cameras and LiDAR: the cameras find painted lane lines through HSV color segmentation, and the LiDAR detects physical obstacles through 3D point cloud processing. This split avoids relying on any single modality for safety-critical obstacle detection.
...
\midrule
Stereo camera (L) & Stereolabs ZED X         & 1920$\times$1200 stereo RGB, 60 fps, GMSL2, wide FOV & Front-left lane detection \\
Stereo camera (F) & Stereolabs ZED X         & 1920$\times$1200 stereo RGB, 60 fps, GMSL2, wide FOV & Front lane detection \\
Stereo camera (R) & Stereolabs ZED X         & 1920$\times$1200 stereo RGB, 60 fps, GMSL2, wide FOV & Front-right lane detection \\
3D LiDAR          & Velodyne VLP-16          & 16 layers; 360$^\circ$; 100\,m range; 10\,Hz         & Obstacle detection \\
GNSS/INS          & Xsens MTi-680G           & 9-axis; 100\,Hz; $\pm 2$--$5$\,m (RTK disabled)      & Localization \\
Compute           & NVIDIA Jetson Orin AGX   & GPU; Ethernet; GMSL2 capture                          & Perception \\
\bottomrule
\end{tabularx}
\end{table}

Per IGVC rules, RTK corrections are not permitted, so the Xsens MTi-680G operates in standalone GNSS/INS mode. This yields approximately $\pm 2$--$5$\,m position accuracy; the localization stack compensates by fusing GNSS with wheel/track odometry and the unit's 9-axis inertial data for short-horizon dead reckoning between waypoints.

\subsection{Lane Detection via HSV Computer Vision}

\textit{ROBOT NAME} uses a classical computer vision approach on the three ZED X stereo cameras to find painted course lines and project them into a drivable-area map. The front-left, front, and front-right ZED X cameras are processed independently and then merged in the vehicle frame. This three-camera arrangement provides overlapping coverage of lane lines directly in front of the robot and on both sides, which is more reliable than a single forward camera when the vehicle is turning or when a lane line falls outside the forward field of view.

Each camera feed is processed through the following pipeline on the Jetson Orin:

\begin{enumerate}[nosep]
    \item \textbf{Camera calibration} --- Correct lens distortion using the factory ZED X intrinsics and a team-calibrated extrinsic from each camera to the vehicle frame.
    \item \textbf{Perspective transform} --- Project the rectified image onto a top-down 2D ground plane using the known mounting geometry.
    \item \textbf{HSV thresholding} --- Convert to HSV color space and threshold on hue/saturation/value to isolate white and colored lane paint while rejecting grass and shadows.
    \item \textbf{Morphological cleanup} --- Apply erosion/dilation to suppress speckle and bridge short gaps in the detected line pixels.
    \item \textbf{Occupancy grid fusion} --- Merge the front-left, front, and front-right lane masks into a unified vehicle-frame lane mask for the planner.
\end{enumerate}

An auto-calibration routine samples the ground at startup and in response to large lighting changes to adapt the HSV thresholds to current conditions.

\subsection{LiDAR-Based Obstacle Detection}

Physical obstacles (barrels, cones, fences, pedestrians) are detected using the Velodyne VLP-16. Point clouds stream to the Jetson Orin over Ethernet at 10\,Hz and are processed through the following pipeline:

\begin{enumerate}[nosep]
    \item \textbf{Point cloud filtering} --- Crop to a region of interest around the vehicle and downsample with a voxel grid.
    \item \textbf{Ground plane removal} --- Estimate and remove the ground plane so that terrain does not register as an obstacle.
    \item \textbf{Clustering} --- Group the remaining non-ground points into discrete obstacle clusters.
    \item \textbf{Occupancy update} --- Project clusters into a 2D occupancy grid aligned with the lane mask from the camera pipeline.
\end{enumerate}

The occupancy grid produced by the LiDAR pipeline and the lane mask produced by the HSV pipeline are combined into a single costmap that the navigation layer consumes for path planning.
...
\textit{ROBOT NAME} uses an extended Kalman filter that fuses the following inputs:
\begin{itemize}[nosep]
    \item \textbf{Track odometry} --- Encoder feedback from the SparkMAX motor controllers, transmitted through the Teensy 4.1 over CAN bus and used for short-horizon dead reckoning.
    \item \textbf{GNSS data} --- Xsens MTi-680G standalone GNSS fix at 5\,Hz. RTK corrections are not used per IGVC rules, giving approximately $\pm 2$--$5$\,m absolute position accuracy.
    \item \textbf{IMU data} --- 9-axis inertial data (3-axis gyro, 3-axis accelerometer, 3-axis magnetometer) from the Xsens MTi-680G at 100\,Hz, providing heading and angular velocity.
\end{itemize}

The Xsens MTi-680G connects directly to the Jetson Orin over USB and runs its internal sensor fusion at 100\,Hz before publishing to the localization node. Because competition rules prohibit RTK, the localization stack is tuned to weight odometry and inertial data more heavily than the raw GNSS fix between waypoint captures, using GNSS primarily to bound drift over longer distances.
